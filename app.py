...
def period_picker(prefix):
    ...
    option = st.selectbox(
        "Periodo",
        ["Todos", "Hoy", "Semana", "Quincena", "Mes", "Año", "Personalizado"],
...
def clear_cache():
    """Limpia caches de Streamlit.

    En esta versión load_df ya no usa cache, pero limpiamos cache_data
    por seguridad para evitar lecturas viejas.
    """
    try:
        st.cache_data.clear()
    except Exception:
...
def load_df(sheet_key):
    ...
    # Evita problemas si Google Sheets quedó con encabezados duplicados.
    headers = []
    seen = {}
    for h in raw_headers:
        ...
        if h in seen:
            seen[h] += 1
            headers.append(f"{h}__duplicada_{seen[h]}")
        else:
            seen[h] = 0
            headers.append(h)
    ...
    # Asegura columnas oficiales aunque la hoja venga de una versión anterior.
    for expected in SHEETS[sheet_key]["headers"]:
...
def append_record_verified(sheet_key, record):
    ...
    # Pequeña pausa para que Streamlit/Sheets terminen de reflejar el cambio.
    time.sleep(0.4)
    clear_cache()
...
def page_gastos():
    ...
    with st.expander("Ver últimos movimientos crudos de Google Sheets"):
        raw_movs = movements_df()
        if raw_movs.empty:
            st.info("Movimientos está vacío.")
        else:
            cols = [c for c in ["fecha_hora", "tipo", "producto", "monto", "pagado_con", "nota"] if c in raw_movs.columns]
            st.dataframe(raw_movs.tail(20)[cols], use_container_width=True)
...
def page_saldos():
    ...
    clear_cache()
    bal = calculate_balances()
    st.caption(f"Movimientos leídos ahora: {len(movements_df())}")
