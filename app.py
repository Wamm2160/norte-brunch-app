...
SHEETS = {
    ...
    "movimientos": {
        ...
        "headers": [
            ...
            "monto", "pagado_con",
            "saldo_resultante", "deuda_resultante",
            "nota",
        ],
        "default_rows": [],
    },
...
NUMERIC_MOVEMENT_COLUMNS = [
    "cantidad", "precio_unitario", "ganancia_personal_unitaria",
    "total_linea", "ganancia_personal_linea", "dinero_norte_linea",
...
def mark_saldo_movement(saldos, fecha_hora=None):
    saldos["ultimo_movimiento_saldo"] = fecha_hora or mx_now().strftime("%Y-%m-%d %H:%M:%S")


def add_movement(record):
    append_record("movimientos", record)


def add_movements(records):
    append_records("movimientos", records)


def set_saldo_manual(new_amount, note):
    saldos = get_saldos()
    previous = saldos["saldo_norte_brunch"]
    new_amount = round(to_float(new_amount), 2)
    delta = round(new_amount - previous, 2)

    saldos["saldo_norte_brunch"] = new_amount
    ...
    add_movement({
        **datetime_record(),
        ...
        "nota": note or f"Ajuste de saldo de {pesos(previous)} a {pesos(new_amount)}",
    })


def set_deuda_manual(new_amount, note):
    saldos = get_saldos()
    previous = saldos["deuda_norte_a_mi"]
    new_amount = round(to_float(new_amount), 2)
    delta = round(new_amount - previous, 2)

    saldos["deuda_norte_a_mi"] = new_amount
    save_saldos(saldos)
    ...
    add_movement({
        **datetime_record(),
        ...
        "nota": note or f"Ajuste de deuda de {pesos(previous)} a {pesos(new_amount)}",
    })
...
def add_line_to_order(pedido_id, product_name, quantity):
    ...
    if match.empty:
        st.error("No encontré el producto seleccionado.")
        return
...
def pay_order(pedido_id, payment_method, payment_date_record, note):
    ...
    total = totals["total"]
    if total <= 0:
        st.error("El total del pedido debe ser mayor que cero.")
        return
    ...
    add_movements(movement_records)
    ...
    mark_saldo_movement(saldos, payment_date_record["fecha_hora"])
    save_saldos(saldos)
...
# -----------------------------
# DATAFRAMES Y REPORTES
# -----------------------------

def movimientos_df():
    df = load_df("movimientos")
    if df.empty:
        return df
...
def ventas_df():
    ...
    if df.empty:
        return df
    return df[df["tipo"].astype(str).str.lower() == "venta"].copy()
...
def gastos_df():
    ...
    if df.empty:
        return df
    return df[df["tipo"].astype(str).str.lower() == "gasto"].copy()


def deuda_movimientos_df():
    df = movimientos_df()
    if df.empty:
        return df
    return df[df["tipo"].astype(str).str.lower().isin(["aumento_deuda", "pago_deuda", "ajuste_deuda"])].copy()
...
def top_report_data(ventas):
    if ventas.empty:
        return None, None, None, pd.DataFrame(), pd.DataFrame()

    day_sales = ventas.groupby("dia_semana").agg(
        ventas=("total_linea", "sum"),
        cantidad=("cantidad", "sum"),
    ).reset_index().sort_values("ventas", ascending=False)

    product_summary = ventas.groupby("producto").agg(
        cantidad=("cantidad", "sum"),
        ventas=("total_linea", "sum"),
        ganancia=("ganancia_personal_linea", "sum"),
    ).reset_index()

    most_day = day_sales.iloc[0] if not day_sales.empty else None
    most_sold = product_summary.sort_values("cantidad", ascending=False).iloc[0] if not product_summary.empty else None
    most_profit = product_summary.sort_values("ganancia", ascending=False).iloc[0] if not product_summary.empty else None

    return most_day, most_sold, most_profit, day_sales, product_summary


def report_for_period(title, start, end):
    ventas = filter_by_dates(ventas_df(), start, end)
    if ventas.empty:
        st.info(f"{title}: no hay ventas.")
        return

    st.subheader(title)

    c1, c2 = st.columns(2)
    c1.metric("Ventas brutas", pesos(ventas["total_linea"].sum()))
    c2.metric("Ganancia personal", pesos(ventas["ganancia_personal_linea"].sum()))

    most_day, most_sold, most_profit, day_sales, product_summary = top_report_data(ventas)

    if most_day is not None:
        st.write(f"**Día que más vende:** {most_day['dia_semana']} con {pesos(most_day['ventas'])}.")
    if most_sold is not None:
        st.write(f"**Producto más vendido:** {most_sold['producto']} con {most_sold['cantidad']:g} unidades.")
    if most_profit is not None:
        st.write(f"**Producto con más ganancia:** {most_profit['producto']} con {pesos(most_profit['ganancia'])}.")

    with st.expander(f"Detalle {title}"):
        st.write("Por producto")
        st.dataframe(product_summary.sort_values("ganancia", ascending=False), use_container_width=True)
...
def page_pedidos():
    ...
    if lines.empty:
        ...
        payment_date = selected_datetime_record(f"pay_{pedido_id}", "Fecha y hora de la venta")
        ...
        if st.button("Cobrar pedido", key=f"pay_{pedido_id}"):
            pay_order(pedido_id, payment_method, payment_date, note)
...
def page_gastos():
    ...
    with st.form("gasto_form"):
        fecha = selected_datetime_record("gasto_form", "Fecha y hora del gasto")
    ...
    if guardar:
        ...
        if pagado_con == "Dinero de Norte Brunch":
            ...
            mark_saldo_movement(saldos, fecha["fecha_hora"])
        else:
        ...
        if pagado_con == "Mi dinero personal":
            records.append({
                **fecha,
                "tipo": "aumento_deuda",
                "movimiento_id": str(uuid.uuid4()),
                ...
                "pagado_con": pagado_con,
                "deuda_resultante": saldos["deuda_norte_a_mi"],
                "nota": f"Gasto pagado personalmente: {nota}",
            })

        add_movements(records)
        st.success("Gasto guardado.")
        st.rerun()

    st.divider()
    start, end = period_picker("gastos")
    df = filter_by_dates(gastos_df(), start, end)
...
def page_saldos():
    ...
    st.caption("Ventas netas contra gastos del negocio. No incluye deuda.")
    ...
    st.caption("Solo deuda y pagos de deuda. No se mezclan pagos de ganancia personal.")

    debt_df = deuda_movimientos_df()
    if not debt_df.empty:
        tipos = debt_df["tipo"].astype(str).str.lower()
        pagos_deuda = debt_df[tipos == "pago_deuda"]["monto"].sum()
        aumentos_deuda = debt_df[tipos.isin(["aumento_deuda", "ajuste_deuda"])]["monto"].sum()
    else:
        pagos_deuda = 0
        aumentos_deuda = 0
    ...
    d3, d4 = st.columns(2)
    d3.metric("Movimientos/ajustes de deuda", pesos(aumentos_deuda))
        ...
        if st.button("Registrar mi pago semanal", disabled=(not permitido or not confirmar)):
            ...
            add_movement({
                **datetime_record(),
                ...
                "monto": amount,
                "saldo_resultante": saldos["saldo_norte_brunch"],
                "nota": f"Pago personal. Diezmo sugerido: {pesos(diezmo)}. Libre estimado: {pesos(libre)}.",
            })
...
def page_reportes():
    ...
    if not ventas.empty:
        ...
        if most_day is not None:
            st.write(f"**Día que más se vende:** {most_day['dia_semana']} con {pesos(most_day['ventas'])}.")
        if most_sold is not None:
            st.write(f"**Producto más vendido:** {most_sold['producto']} con {most_sold['cantidad']:g} unidades.")
        if most_profit is not None:
            st.write(f"**Producto con más ganancia:** {most_profit['producto']} con {pesos(most_profit['ganancia'])}.")
    ...
    st.divider()
    st.subheader("Comparativo por periodo")
    periods = {
        "Semana": period_dates("Semana"),
        "Quincena": period_dates("Quincena"),
        "Mes": period_dates("Mes"),
        "Año": period_dates("Año"),
    }

    for title, (p_start, p_end) in periods.items():
        with st.expander(title):
            report_for_period(title, p_start, p_end)
...
def page_diagnostico():
    ...
    st.caption("Usa esta página solo si algo no cuadra o si quieres revisar la estructura.")
